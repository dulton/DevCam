...
void CALLBACK DecCallBackFun(long nPort,char* pBuf,long nSize,FRAME_INFO* pFrameInfo, long nUser, long nReserved2){
	unsigned char* buf = (unsigned char*)nUser;
...
void CALLBACK RealDataCallBack(LONG lRealHandle, DWORD dwDataType, BYTE *pBuffer,DWORD dwBufSize,DWORD dwUser){
	LPCommonParam lpCommonParam = (LPCommonParam)dwUser;
	switch (dwDataType){
	case NET_DVR_SYSHEAD: //系统头
		if (!PlayM4_GetPort((long*)&lpCommonParam->playPort)){  //获取播放库未使用的通道号
			break;
		}
		//m_iPort = lPort; //第一次回调的是系统头，将获取的播放库port号赋值给全局port，下次回调数据时即使用此port号播放
		if (dwBufSize > 0){
			if (!PlayM4_SetStreamOpenMode(lpCommonParam->playPort, STREAME_REALTIME)){  //设置实时流播放模式
				break;
			}

			if (!PlayM4_OpenStream(lpCommonParam->playPort, pBuffer, dwBufSize, 1024*100000)){ //打开流接口
				break;
			}

			PlayM4_SetDecCBStream(lpCommonParam->playPort,1);

			PlayM4_SetDecCallBackMend(lpCommonParam->playPort,DecCallBackFun,(long)lpCommonParam->frameBuffer);

			if (!PlayM4_Play(lpCommonParam->playPort, NULL)){ //播放开始
				break;
			}
		}
	case NET_DVR_STREAMDATA:   //码流数据
		if (dwBufSize > 0 && lpCommonParam->playPort != -1){
			if (!PlayM4_InputData(lpCommonParam->playPort, pBuffer, dwBufSize)){
				break;
			} 
		}
	}
...
_EXT_C_DEVCAM_API BOOL CAM_StartStreamPlay(CommonParam *commonParam,long channel){
	...
	if(commonParam->deviceType == 0){
		...
		NET_DVR_SetRealDataCallBack(commonParam->playHandle,RealDataCallBack,(DWORD)commonParam);
	}
...
_EXT_C_DEVCAM_API BOOL CAM_CaptureFrame(CommonParam *commonParam,long channel){
	...
	NET_DVR_JPEGPARA jpegPara = {255,0};
	if(commonParam->deviceType == 0){
		commonParam->JPEGSize = put_jpeg_yv12_memory(commonParam->JPEGBuffer,commonParam->frameBuffer,commonParam->frameWidth,commonParam->frameHeight,100);
	}
